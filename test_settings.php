...
$PAGE->set_url($CFG->wwwroot . '/local/eschoolapi/test_settings.php');
...
<div style='display: flex; flex-direction: column; width: fit-content'>
    <div class="lp-grid">
        <label class="row1 col1">Режим наследования:</label>
        <select class="row1 col2" id="inheritanceType">
            <option value="0" selected>Полное</option>
            <option value="1">Частичное</option>
            <option value="2">Отключено</option>
        </select>
    </div>
    <div style="display: flex; flex-direction: column" id="inheritanceSettings">
        <div style="padding-bottom: 15px">Параметры наследования:</div>
        <div class="lp-grid" style="margin-left: 30px">
            <label class="col1"><?php echo get_string('mark_autoexport', $plugin->component) ?>:</label>
            <select id=enable" class="col2">
            ...
            <label class="col1">Триггер:</label>
            <select class="col2" id="inheritTrigger">
                <option>Наследовать</option>
                <option>Не наследовать</option>
            </select>
            <label class="col1">Параметры оценивания:</label>
            <select class="col2" id="inheritMarking">
                <option>Наследовать</option>
                <option>Не наследовать</option>
            </select>
        </div>
    ...
    <div style="display: flex; flex-direction: column" id="trigger_settings">
        ...
        <div id="trigger_settings_onfinishparams" class="lp-grid hidden" style="margin-left: 30px">
            <label class="row1 col1">Режим оценивания:</label>
            <select id="trigger_settings_onfinishparams_marking" class="row1 col2">
                <option selected>Первая попытка</option>
                <option>Каждая попытка (Обновление)</option>
            </select>
            <label class="row2 col1">При закрытии теста:</label>
            <select id="trigger_settings_onfinishparams_on_test_close" class="row2 col2">
                <option>Обнулить не завершённые попытки</option>
                <option>Не учитывать не завершённые попытки</option>
            </select>
        </div>
        <div id="trigger_settings_ondateparams" class="lp-grid" style="margin-left: 30px">
            <label class="col1">Дата выставления:</label>
    ...
    <div style="display: flex; flex-direction: column; margin-bottom: 15px">
        <div style="padding-bottom: 15px">Параметры выставления оценок:</div>
        <div class="lp-grid" style="margin-left: 30px" id="classDateMapping">
            <div class="col1">Класс (Группа)</div>
            <div class="col2">Дата в электронной школе</div>
        </div>
        <button style="margin-left: 30px" id="addMapping">Добавить соотношение Дата <=> Группа</button>
    </div>
...
<script>
    function onLoad() {
        ...
        const clazzes = [
            <?php
            $user = $USER->id;
            $classes = $DB->get_recordset_sql('SELECT class FROM {local_eschoolapi_tcm} WHERE userid = ?', array($user));
        ...
        document.getElementById('addMapping').addEventListener('click', (e) => {
            let mappingId = mapping.length;
            let clazz = document.createElement('select');
